...
if __name__ == '__main__':
    ...
    data = WordFile("../Data/Extracted/" + collection + "_" + year + "_class_words.psv")
    ...
    #g.V().hasLabel('class').drop().iterate()
    ...
    for entry in data:
        ...
        if entry_text not in subject_ids:
            V = g.addV('class').property('id', entry_hash).property('name', entry_text).next()
        ...
        g.V(buffer_list[0][0]).addE('has_class').to(__.V(buffer_list[0][1])).property('weight', 1) \
         .V(buffer_list[1][0]).addE('has_class').to(__.V(buffer_list[1][1])).property('weight', 1) \
         .V(buffer_list[2][0]).addE('has_class').to(__.V(buffer_list[2][1])).property('weight', 1) \
         .V(buffer_list[3][0]).addE('has_class').to(__.V(buffer_list[3][1])).property('weight', 1) \
         .V(buffer_list[4][0]).addE('has_class').to(__.V(buffer_list[4][1])).property('weight', 1) \
         .iterate()
    ...
    for b in buffer_list:
        g.V(b[0]).addE('has_class').to(__.V(b[1])).property('weight', 1).next()
